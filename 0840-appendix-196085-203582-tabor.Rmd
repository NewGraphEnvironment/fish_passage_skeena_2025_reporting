...
```{r map-196085, fig.cap = my_caption, eval = T}
 my_caption <- "Map of Tabor Creek"
 
 knitr::include_graphics("fig/gis/map_tabor.jpeg")
...
```{r tab-edna-results-196085-prep, include=FALSE}
...
edna_results_196085 <- edna_bystargets_fraser |>
  dplyr::filter(
    grepl(paste0("^(", my_site, "|", my_site2, ")_"), site_id),
    !control_blank_field,
...
Detection results are presented in Table \@ref(tab:tab-edna-results-196085). Format is `CODE(max_droplets)`, with `*` flagging samples UNBC reran. Detected means at least 4 positive droplets, the standard ddPCR call threshold; sub-threshold signals of 1-3 droplets are not interpretable as species presence. Site-level detail for the whole project, along with field blanks and retests, is in [Appendix - Environmental DNA Results](#app-edna).

<br>

```{r tab-edna-results-196085}
edna_results_196085 |>
  fpr::fpr_kable(
    caption_text = paste0("eDNA detection results at PSCIS crossings ", my_site, " and ", my_site2, "."),
    scroll = FALSE
  )
```
...
The channel above the railway crossings was dewatered at the time of assessment, with the farthest downstream residual pool holding numerous fry and environmental DNA confirming chinook salmon and rainbow trout immediately upstream. Reaches that go dry are easily read as low value, and the literature points the other way. Residual pools within intermittent systems can support high juvenile growth rates and elevated overwinter survival, and both @ebersole_etal2006JuvenileCoho and @wigingtonjr_etal2006Cohosalmon measured smolts emerging from intermittent tributaries as considerably larger than those rearing in perennial habitat, with correspondingly greater survival — outcomes attributed to lower competitive densities and greater food availability [@maslin_mckinnev1998IntermittentStreams]. That work concerns coho salmon, so its application to the chinook rearing above the Tabor Creek railway crossings is an extension rather than a direct read-across, but the situation is the same one those studies describe.

<br>

If that is what is occurring here, the value of the reach depends on whether those fish can leave it when flows resume, which makes the timing of connectivity the question worth monitoring rather than the 0.3 m outlet drop considered on its own. Tracking flow resumption, pool persistence and fish movement across a season would establish whether the reach is producing large, high-survival migrants or stranding a cohort each year. Where fish are found to be trapped, annual salvage is available as a short-term measure. Confirmation that chinook depend on this reach would also give the Canadian National Railway a concrete basis for considering remediation of the railway crossings.
