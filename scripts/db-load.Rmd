...
sites_repo_p2 <- sites_pscis_raw  |> 
  # add the missing p2 ids
  dplyr::mutate(
    source = 
      dplyr::case_when(
        my_crossing_reference %in% ids_p2_missing ~ "phase2",
        T ~ source
      )
  ) |> 
  # drop this b/c we need some raw coords from 2024 data anyway
  sf::st_drop_geometry() |> 
  # here we are dropping the phase 1s!!
  dplyr::filter(
    ...
    habitat_confirmation = 
      dplyr::case_when(
        stringr::str_detect(source, "phase2") | 
          pscis_crossing_id %in% unique(sites_p2$pscis_crossing_id)  ~ "yes",
        T ~ NA_character_),
    design =
      dplyr::case_when(
        pscis_crossing_id %in% ids_design ~ "yes",
        T ~ NA_character_),
    remediation =
      dplyr::case_when(
        pscis_crossing_id %in% ids_remediation ~ "yes",
        T ~ NA_character_),
    fish_sampling = 
      dplyr::case_when(
        pscis_crossing_id %in% unique(fish_sampling$site) ~ "yes",
        T ~ NA_character_),
  ...
  # need to arrang for the fill to work
  dplyr::arrange(pscis_crossing_id) |> 
  dplyr::group_by(pscis_crossing_id) |> 
  tidyr::fill(reassessment:fish_sampling, .direction = "downup") |> 
  # regroup to get ready to pivot the unique links
...
# we have a ghost crossing in the mix without either ID - likely due to incorrect type (string) in my_crossing_reference
# will need to remove
...
sites_all_summary <- sites_all |> 
  ...
  dplyr::rename(UAV = Uav) |> 
  janitor::adorn_totals()
```


```{r tab-sites-sum}
# left this here to view results
sites_all_summary |> 
  my_dt_table(
    page_length = 20,
    cols_freeze_left = 0
...
# 
# test <- t5 |> 
#   dplyr::filter(!is.na(link_uav1))
# 
# test <- t |> 
#   dplyr::filter(stream_name == "Richfield Creek")
# 
# test <- sites_pscis_raw |> 
#   dplyr::filter(stream_name == "Richfield Creek")
#   
# 
# 
