...
dupes_pscis_all <- sites_repo_p1_prep2 |> 
  ...
  # this was backwards so now have switched (F for T)
...
# hashed out to test smarter dupe removal
...
dupes_pull <- dupes_pscis_all |> dplyr::filter(remove == TRUE) |> dplyr::pull(stream_crossing_id)
...
sites_repo_p1 <-  sites_repo_p1_prep2 |>
  ...
  dplyr::filter(
    !stream_crossing_id %in% dupes_pull
  )
