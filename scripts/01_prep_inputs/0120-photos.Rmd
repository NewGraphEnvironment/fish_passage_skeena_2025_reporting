...
```{r def-var, eval = TRUE}
dir_photos_shared = "/Users/airvine/Projects/gis/sern_peace_fwcp_2023/ignore_mobile/photos_resized"
...
form_pscis <- fpr::fpr_sp_gpkg_backup(
  path = fs::path(dir_project, 'form_pscis.gpkg'),
...
```{r dir-create1}
fs::dir_create(dir_photos_shared, recursive = T)
```
...
fpr_photo_resize_batch(
  dir_source = fs::path(path.expand('~/'), 'Projects/gis', params$gis_name, 'ignore_mobile/photos/'),
  dir_target = fs::path(path.expand('~/'), 'Projects/gis', params$gis_name, 'ignore_mobile/photos_resized/')
)

# could erase photos here but will do by hand for safety
# recreate the photos.txt file so the form still works
fs::file_create('/Users/airvine/Projects/gis', params$gis_name, 'ignore_mobile/photos/photos.txt')

# back the photos up to onedrive (should remove all from mergin)
fpr::fpr_photo_resize_batch(
  dir_source = fs::path(path.expand('~/'), 'Projects/gis/sern_skeena_2023/ignore_mobile/photos/photos_resized/'),
  dir_target = fs::path(path.expand('~/'), 'Library/CloudStorage/OneDrive-Personal/Projects/repo/fish_passage_skeena_2023_reporting/data/photos/mergin/')
)
```
...
fpr::fpr_photo_rename(
  ...
  dir_from_stub = fs::path(path.expand('~/'), 'Library/CloudStorage/OneDrive-Personal/Projects/repo/fish_passage_skeena_2023_reporting/data/photos/mergin/'),
  dir_to_stub = fs::path(path.expand('~/'), 'Library/CloudStorage/OneDrive-Personal/Projects/repo/fish_passage_skeena_2023_reporting/data/photos/sorted/',
  col_string_add = TRUE)
)
