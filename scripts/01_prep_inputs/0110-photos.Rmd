...
dir_photos_originals <- fs::path_expand(fs::path("~/Library/CloudStorage/OneDrive-Personal/Projects/2025_data/photos/"))
# The skeena and fraser projects are part of the same contract/job so there is one folder for both on Onedrive. We need to keep the photos separate though so this case when creates separate folders within the data folder.
dir_photos_onedrive <- dplyr::case_when(params$project_region != "peace" ~ fs::path("~/Library/CloudStorage/OneDrive-Personal/Projects", params$job_name, "data/photos/",  params$project_region))
```

# Resize all

We are going to resize the original photos on onedrive. For 2025, all the extra photos are store together in `OneDrive-Personal/Projects/2025_data/photos`, so we will just resize them and then sort them into there project specific folders after. Last year we sorted first then resized, but doing it the other way around means we only have to run the resize script once. 

```{r dir-create}
fs::dir_create(fs::path(dir_photos_originals, "ai/resized"), recurse = TRUE)
fs::dir_create(fs::path(dir_photos_originals, "ls/resized"), recurse = TRUE)
```

```{r shared-dir-resize}
fpr::fpr_photo_resize_batch(
  dir_source = fs::path(dir_photos_originals,"ai/originals"),
  dir_target = fs::path(dir_photos_originals, "ai/resized")
)

fpr::fpr_photo_resize_batch(
  dir_source = fs::path(dir_photos_originals,"ls/originals"),
  dir_target = fs::path(dir_photos_originals, "ls/resized")
)
...
range(s)
# [1] 0.1173391 0.5262947 Not bad.  Lets resize anyway so that we know they fit the reporting
...
fpr::fpr_photo_resize_batch(
  ...
  dir_target = dir_photos_mergin_resized
)
...
##!!!!!!!!!!!!!!!!!!!! special case start see issue https://github.com/NewGraphEnvironment/dff-2022/issues/183
# run in October 2025 for Peace and Fraser to deal with issue linked above
fpr::fpr_photo_resize_batch(
  dir_source = paste0("~/Projects/gis/", params$gis_project_name ,"/photos"),
  dir_target = fs::path(dir_photos_mergin_resized)
)

fs::dir_delete(paste0("~/Projects/gis/", params$gis_project_name ,"/photos"))
# push to mergin - record version number of mergin project in issue to track
##!!!!!!!!!!!!!!!!!!!! special case end
```

# PSCIS site - Rename and move pscis site photos

```{r create-pscis-site-dir}

form_new <- fs::path_expand(fs::path("~/Projects/gis/", params$gis_project_name, '/data_field/', params$project_year, paste0("form_pscis_", params$project_year, ".gpkg")))
...
# We didn't do this in 2024, instead only sorted the photos on Onedrive. Not running for now, Oct 2025. 
...
# create site photo directories on onedrive for the renamed photos
form_pscis_photos_raw |>
  dplyr::pull(site_id) |>
  as.character() |>
  purrr::map(
    fpr::fpr_photo_folders, path = paste0(dir_photos_onedrive, "/renamed/")
  )

# create site photo directories on onedrive for the extra photos
form_pscis_photos_raw |>
  dplyr::pull(site_id) |>
  as.character() |>
  purrr::map(
    fpr::fpr_photo_folders, path = paste0(dir_photos_onedrive, "/extra/")
  )
```
...
# FISS Site - Rename and move Fiss site photos
...
```{r create-fiss-site-dir}

form_new <- fs::path_expand(fs::path("~/Projects/gis/", params$gis_project_name, '/data_field/', params$project_year, paste0("form_fiss_site_", params$project_year, ".gpkg")))
...
# Monitoring site - Rename and move Monitoring site photos

```{r create-mon-site-dir}

form_new <- fs::path_expand(fs::path("~/Projects/gis/", params$gis_project_name, '/data_field/', params$project_year, paste0("form_monitoring_", params$project_year, ".gpkg")))


form_mon_photos_raw <- fpr::fpr_sp_gpkg_backup(
  form_new,
  update_site_id = TRUE,
  # turned this off for now but was on first time
  write_back_to_path = FALSE,
...
# create site photo directories on onedrive for the renamed photo
form_mon_photos_raw |>
  dplyr::pull(site_id) |>
  as.character() |>
  purrr::map(
...
# eDNA site - Rename and move eDNA site photos

```{r create-edna-site-dir}

form_new <- fs::path_expand(fs::path("~/Projects/gis/", params$gis_project_name, '/data_field/', params$project_year, paste0("form_edna_", params$project_year, ".gpkg")))

form_edna_photos_raw <- fpr::fpr_sp_gpkg_backup(
  form_new,
...
# Separate out the side name and create the photo_tag_site column for the photos 
form_edna_photos <- form_edna_photos_raw |> 
  dplyr::mutate(site_id = stringr::str_to_lower(site_id)) |> 
  tidyr::separate(site_id, into = c('site', 'photo_tag_site'),
                  extra = "merge", 
                  fill = "right", 
                  remove = FALSE) |> 
