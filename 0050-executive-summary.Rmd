...
These are not separate studies. Each activity answers a different part of one question: what condition are these watersheds in, and where will restoration effort do the most good. Fish passage assessments and the provincial barrier model describe where fish can currently reach. Habitat confirmations, fish sampling and environmental DNA describe where fish are and what the habitat could support. Floodplain delineation and aerial imagery describe how infrastructure has reshaped the valley bottom that the stream depends on. Climate departure and water temperature describe the trajectory the system is on, and how the flows that formed these channels are changing. Effectiveness monitoring closes the loop, testing whether completed work delivered what it was designed to deliver.

<br>

Assembling these layers is a way of being explicit about what is known, what is not, and what should be measured next, so that restoration decisions rest on evidence rather than assumption. Making that evidence usable is treated as part of the work rather than a by-product: field data are collected on structured digital forms, methods are documented alongside the code that implements them, spatial layers are published to open catalogues instead of being held in project folders, and reporting is written so that partners, First Nations, regulators and tenure holders can read it, question it and build on it. Each year's work is intended to be legible to the next, so understanding accumulates across the program rather than being rebuilt each season.
