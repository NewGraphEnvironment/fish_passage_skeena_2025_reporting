...
Habitat confirmation assessments were conducted at `r n_hab_conf_sites` sites within the `r wsg_names_hab_conf |> knitr::combine_words()` watershed groups, covering approximately `r len_surveyed_km` km of stream length. Detailed habitat metrics were documented alongside eDNA samples to evaluate habitat quality and presence of fish species. 
...
Preliminary top remediation priorities are provided by watershed group. A single definitive ranking is not achievable, because the criteria that determine whether a crossing is worth remediating are not measured in common units, and the deciding constraint sits outside the project in the capacity and willingness of infrastructure owners and tenure holders to support implementation across multi-year timelines. What can be done systematically is to maintain a well-characterised set of candidate sites, so that when a tenure holder is in a position to act, the ecological value of the opportunity and the appropriate scope of work are already understood.
...
- Continue and extend the long-term effectiveness monitoring program, and share what it shows — including where a remediation has not delivered the outcome intended — so decisions elsewhere are better informed.
