...
- Continue the effectiveness monitoring program at PSCIS crossings 197962 (Peacock Creek, Morice FSR) and 124421 (Waterfall Creek, 11th Avenue). Monitoring is where the program learns whether a remediation delivered what it was designed to deliver, and that is only visible over multiple years. Documenting an outcome that did not hold is as valuable as documenting one that did.
...
- Feed monitoring results back into prioritization and design. The purpose of a monitoring program is adaptive management, and what is learned at Peacock Creek and Waterfall Creek should change which treatments are proposed elsewhere.
...
- Resolve the amplification control gap. Internal controls were reported for only a minority of `r params$project_year` samples. Where they are absent, a non-detection cannot be distinguished from a reaction that failed or was inhibited, which limits how far the negative results can be read. Detections are unaffected. Reporting controls consistently is the single change that would most improve the interpretability of this dataset.
