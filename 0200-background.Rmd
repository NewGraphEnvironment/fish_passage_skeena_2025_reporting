...
The Skeena study area includes the Bulkley River, Zymoetz River, Kispiox River, Morice River and Kitsumkalum River watershed groups (Figure \@ref(fig:overview-map)) and is within the traditional territories of the Wet'suwet'en, Gitxsan and Tsimshian.
...
fiss_sum_wshed_filter <- fiss_sum |>
  ...
  dplyr::mutate(upstream_area_km = upstream_area_ha / 100)

bin_1 <- 0
bin_n <- ceiling(max(fiss_sum_wshed_filter$upstream_area_km, na.rm = TRUE) / 5) * 5
bins <- seq(bin_1, bin_n, by = 5)

plot_wshed_hist <- ggplot2::ggplot(fiss_sum_wshed_filter, ggplot2::aes(x = upstream_area_km)) +
  ggplot2::geom_histogram(
    breaks = bins,
    position = "identity",
    size = 0.75
  ) +
  ggplot2::labs(x = "Upstream Watershed Area (km)", y = "Count Fish (#)") +
  ggplot2::facet_wrap(~species_code, ncol = 2) +
  ggplot2::theme_bw(base_size = 11) +
