...
## Session 2026-08-02 — post-review pass and PR

Work after Phase 10 that came out of the author's read of the draft.

**Correction to the Phase 10 entry above.** That entry says the eleven canonical bib entries "had to
be appended by hand" because `update_bib` was FALSE. That was not the workflow and the author said
so. `references.bib` is generated by rbbt from Better BibTeX and is never hand-edited. It was
regenerated properly with `update_bib: TRUE` and Zotero running — all 93 keys resolved, and the
regenerated file differed from the hand-edited one by 431 insertions / 461 deletions. The
hand-edited version is gone from the branch.

**Bittner Creek effectiveness monitoring** — added `0860-appendix-196200-bittner.Rmd`, citing our
2022 MoTI monitoring report, carrying its two amalgamated culvert photos (2014 / 2022) and the
lidar figure, and setting out the three-way disagreement between the 2023 salvage record, the 2025
eDNA result and the current structure condition.

**The Bittner eDNA zero is not an absence.** Chased it down: the amplification controls for that
sample were never run (`sample_lambda_ok` / `sample_ep_ok` FALSE, positives NA) and rainbow trout is
not on the panel that was run. Program-wide, 33 of 38 Fraser samples are in the same position, as
are 22 of the 25 sites that *did* produce detections. The controls appear only on the workbook's
PRIORITY sheets and never on the dated run sheets — 26 of 104 samples — with the same assay panels
on both sides, so this looks like a worksheet-structure problem and the data may be recoverable.
Filed as #15 and drafted a question to UNBC in `compost`. Those zeros are reported as untested, not
as absences.

**Stock assessment moved out of Background** into `0700-appendix-stock-assessment.Rmd` — a
1,486-row NuSEDS table does not belong in a body chapter.

**HCTF credited.** Title block names the Habitat Conservation Trust Foundation with CAT26-0-636
above MoTI, the header carries the combined SERNbc/HCTF logo from the Skeena 2024 reporting, and
the introduction and executive summary set out the funding history — SERNbc-internal and MoTI
support produced the region's 2023 report, and the project is now also supported by HCTF alongside
Skeena as the Northern British Columbia Fish Passage Restoration project. No version bump: 0.3.0 is
not published yet.

**Two more defects fixed en route.** `stringr::str_like()` became case-sensitive in stringr 1.5.0,
which had been rendering Table 4.5 completely empty in the published report — confirmed against
`git show HEAD:docs/`. And the introduction pointed print readers at
`fish_passage_peace_2024_reporting.pdf`, a file from a different region's repository; the name is
now derived from `_bookdown.yml`.

Branch pushed at 25 commits, PR opened closing #10 and #11. `/planning-archive` after merge.
