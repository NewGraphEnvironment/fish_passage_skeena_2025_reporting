...
The modelled result is not regenerated for this report. It is published to the `stac-floodplains-bc` collection on the spatial catalogue and read from there, so one modelled product serves every report that needs it rather than each repository carrying a copy of its own. Full methodology and watershed-scale mapping are in [Appendix - Floodplain Delineation](#app-floodplain).
